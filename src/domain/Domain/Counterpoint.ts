...
class OnlyChordTones implements CounterPointRule {
	private readonly harmonizer: ScaleHarmonizer = new TriadHarmonizer(
	...
	validate(parts: CounterPointParts): CounterPointRuleStatus {
		...
		for (const note of parts.counterPoint.phrase) {
			...
			if (!isChordTone) {
				return { isValid: false, message: "not a chord tone", index };
			}
	...
	validate(parts: CounterPointParts): CounterPointRuleStatus {
		...
		for (const note of parts.counterPoint.phrase) {
			if (note.Duration !== Duration.Whole) {
				return { isValid: false, message: "not a whole note", index };
			}
	...
	validate(parts: CounterPointParts): CounterPointRuleStatus {
		...
		for (const note of parts.counterPoint.phrase) {
			if (
				...
				return { isValid: false, message: "not in range", index };
			}
	...
	validate(parts: CounterPointParts): CounterPointRuleStatus {
		...
		for (const note of parts.counterPoint.phrase) {
			if (previous && note.isSamePitch(previous)) {
				return { isValid: false, message: "repeated note", index };
			}
			previous = note;
			index++;
		}
	...
	validate(parts: CounterPointParts): CounterPointRuleStatus {
		...
		for (const note of parts.counterPoint.phrase) {
			if (previous && note.intervalTo(previous).isLargarThan(Interval.MajorSixth)) {
				return { isValid: false, message: "invalid leap", index };
			}
			previous = note;
			index++;
		}
